...
const {query, create, remove, update} = channels;
...
export default {
    ...
    effects: {
        ...
        *create({payload}, {call, select, put}) {
            ...
            const {total, current, list}= yield select(({channels: {total, current, list}}) => ({total, current, list}));
        ...
        *update({payload}, {select, call, put}) {
            yield put({type: 'hideModal'});
            yield put({type: 'showLoading'});
            const id = yield select(({channels}) => channels.currentItem.id);
